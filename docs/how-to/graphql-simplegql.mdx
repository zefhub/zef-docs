...
run:
```
python3 -m zef.gql.simplegql sample.graphql data-graph --create --port 5001 --bind 0.0.0.0
```

You may then point your browser at the [GraphQL Playground](https://graphqlbin.com) and enter
`http://localhost:5001/gql` to explore the GraphQL API. Note that the graph
created `data-graph` contains no data.

For help on the commandline arguments, run:
